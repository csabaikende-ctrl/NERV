...
for i in "${FIRMWARES[@]}"; do
    ...
    LATEST_FIRMWARE="$(GET_LATEST_FIRMWARE "$MODEL" "$CSC")"; [ "$MODEL" = "SM-S911B" ] && LATEST_FIRMWARE="S911BXXS8DYI3/S911BOXM8DYI3/S911BXXS8DYI3"
    ...
    while true; do
        ...
        (
        cd "$OUT_DIR"
        samloader -m "$MODEL" -r "$CSC" -i "$IMEI" -s "$SERIAL_NO" download $([ "$MODEL" = "SM-S911B" ] && echo '-v "S911BXXS8DYI3/S911BOXM8DYI3/S911BXXS8DYI3"') -O "$ODIN_DIR/${MODEL}_${CSC}" 1> /dev/null || exit 1
        )
